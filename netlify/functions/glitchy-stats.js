    ...
    if (endDate >= today) {
      ...
      if (supabase) {
        ...
        try {
          ...
          // Live Performance graph ONLY: snapshot the combined total-so-far,
          // interpolating across any gap since the last recorded hour — see
          // earningsSnapshotToday's own comment for why (confirmed live:
          // Glitchy's entries carry no per-entry timestamps, only a bare
          // day, so unlike Spend there's no true hourly breakdown available
          // at all, only this best-effort smoothing).
